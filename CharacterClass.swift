...
class Character {
    ...
    var HP: Double
    var baseHP: Double
    let trueBaseHP: Double
    var hpDifferential: Double = 0
    var damage: Double
    var baseDamage: Double
    let trueBaseDamage: Double
    var damageDifferential: Double = 0
    ...
    convenience init (characterName: String, isEnemy: Bool, characterBaseImage: UIImage, imageNumberAttack: Int, imageNumberMovement: Int, movementSpeed: Double, HP: Double, attackSpeed: Double, damage: Double, armorValue: Double, isMelee: Bool, isHealer: Bool, abilityNames: [String], equipmentType: Int) {
        self.init (characterName: characterName, isEnemy: isEnemy, characterBaseImage: characterBaseImage, imageNumberAttack: imageNumberAttack, imageNumberMovement: imageNumberMovement, imageNumberProjectile: 0, imageNumberHeal: 0, movementSpeed: movementSpeed, HP: HP, attackSpeed: attackSpeed, damage: damage, armorValue: armorValue, isMelee: isMelee, isHealer: isHealer, secondaryBaseImage: nil, abilityNames: abilityNames, equipmentType: equipmentType)
    }
    
    init (characterName: String, isEnemy: Bool, characterBaseImage: UIImage, imageNumberAttack: Int, imageNumberMovement: Int, imageNumberProjectile: Int, imageNumberHeal: Int, movementSpeed: Double, HP: Double, attackSpeed: Double, damage: Double, armorValue: Double, isMelee: Bool, isHealer: Bool, secondaryBaseImage: UIImage?, abilityNames: [String], equipmentType: Int) {
        name = characterName
        ...
        if (self.isEnemy == false && self.name != "0") {
            self.dragImage.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(showAbilities(_:))))
        }
    ...
    @objc func showAbilities(_ sender: UITapGestureRecognizer) {
        print("Entered Button onClick")
//        var diffCheck: CGFloat = 0.0
//        var diffDoubleCheck: CGFloat = -1.0
        for i in 0...3 {
            if (activeHeroes[i].dragImage.center == sender.view!.center) {
//                diffCheck = abs(activeHeroes[i].imageView.layer.presentation()!.frame.midY - sender.view!.center.y) + abs(activeHeroes[i].imageView.layer.presentation()!.frame.midX - sender.view!.center.x)
//                if (diffCheck > diffDoubleCheck) {
//                    diffDoubleCheck = diffCheck
...
//                }
        ...
        print("Selected Abilities: \(selectedHero.name)")
